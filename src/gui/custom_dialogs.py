...
def show_validation_report(parent, github_mods, gdrive_mods, other_domains, failed_list):
    ...
    if len(github_mods) > 0:
        ...
        tk.Label(github_frame, text=f"✓ {len(github_mods)} mod(s) from GitHub", 
                font=("Arial", 11, "bold"), fg=TriOSTheme.GITHUB_FG).pack(anchor=tk.W)
        
        # List GitHub mods
        github_list_frame = tk.Frame(github_frame, relief=tk.SUNKEN, bd=1, bg=TriOSTheme.GITHUB_BG)
        ...
        github_text = tk.Text(github_list_frame, height=min(4, len(github_mods)), width=55,
                             font=("Courier", 9), wrap=tk.WORD, bg=TriOSTheme.GITHUB_BG, fg=TriOSTheme.TEXT_PRIMARY, relief=tk.FLAT)
    ...
    if len(gdrive_mods) > 0:
        ...
        tk.Label(gdrive_frame, text=f"✓ {len(gdrive_mods)} mod(s) from Google Drive", 
                font=("Arial", 11, "bold"), fg=TriOSTheme.GDRIVE_FG).pack(anchor=tk.W)
        ...
        info_text = tk.Label(gdrive_frame, 
            ...
            font=("Arial", 9, "italic"), fg=TriOSTheme.TEXT_SECONDARY, wraplength=450, justify=tk.LEFT)
        info_text.pack(anchor=tk.W, padx=(20, 0), pady=(2, 0))
        
        # List Google Drive mods
        gdrive_list_frame = tk.Frame(gdrive_frame, relief=tk.SUNKEN, bd=1, bg=TriOSTheme.GDRIVE_BG)
        ...
        gdrive_text = tk.Text(gdrive_list_frame, height=min(4, len(gdrive_mods)), width=55,
                             font=("Courier", 9), wrap=tk.WORD, bg=TriOSTheme.GDRIVE_BG, fg=TriOSTheme.TEXT_PRIMARY, relief=tk.FLAT)
    ...
    if other_domains:
        ...
        tk.Label(other_frame, text=f"⚠ {total_other} mod(s) from other sources", 
                font=("Arial", 11, "bold"), fg=TriOSTheme.OTHER_FG).pack(anchor=tk.W)
        
        # List each domain with its mods
        other_list_frame = tk.Frame(other_frame, relief=tk.SUNKEN, bd=1, bg=TriOSTheme.OTHER_BG)
        other_list_frame.pack(fill=tk.X, padx=(20, 0), pady=(4, 0))
        
        other_text = tk.Text(other_list_frame, height=min(5, total_other), width=55,
                            font=("Courier", 9), wrap=tk.WORD, bg=TriOSTheme.OTHER_BG, fg=TriOSTheme.TEXT_PRIMARY, relief=tk.FLAT)
    ...
    if failed_list:
        ...
        tk.Label(failed_frame, text=f"✗ {len(failed_list)} mod(s) inaccessible", 
                font=("Arial", 11, "bold"), fg=TriOSTheme.FAILED_FG).pack(anchor=tk.W, pady=(0, 2))
        
        tk.Label(failed_frame, 
            text="These mods cannot be downloaded. Check URLs or contact mod authors.",
            font=("Arial", 9, "italic"), fg=TriOSTheme.TEXT_SECONDARY, wraplength=450, justify=tk.LEFT).pack(anchor=tk.W, padx=(20, 0), pady=(2, 0))
        
        # Scrollable list of failed mods
        failed_list_frame = tk.Frame(failed_frame, relief=tk.SUNKEN, bd=1, bg=TriOSTheme.FAILED_BG)
        ...
        failed_text = tk.Text(failed_list_frame, height=min(5, len(failed_list)), width=55, 
                             font=("Courier", 9), wrap=tk.WORD, bg=TriOSTheme.FAILED_BG, fg=TriOSTheme.TEXT_PRIMARY, relief=tk.FLAT)
